...
**Entregável da Fase 0** (Documento 2, §34.6) · **Estado:** PROVISÓRIO — pendente de verificação
em fonte oficial acessível
**Data:** 2026-07-27 · **Autor:** agente
...
**d) Efeito da flag.** Declarar `isMonitoringTool` faz o Google Play informar ao usuário, por
*warning string* do Play Protect, que **há um aplicativo de monitoramento no aparelho**.

`VERIFICAR:` a *warning string* do Play Protect é acionada pela flag, e com que texto?
...
7. **Somente maiores de 18.** O módulo familiar com menores — que é justamente a hipótese
   "pais→filhos" — **muda a classificação** e exige reavaliação deste parecer antes de existir.

O item 7 merece destaque: o Documento 1, §22 prevê "plano familiar entre adultos" e o Documento 3,
§46 exige avaliação específica para menores. **Se um dia o produto incluir menores, ele passa a
poder ser um aplicativo de monitoramento aceitável — e aí a notificação persistente, o ícone único
e a flag passam a ser obrigatórios.** É a única hipótese em que a categoria ajuda em vez de matar.
...
### 5.2 Para a flag `isMonitoringTool` — recomendação, não decisão

> **Recomendação: NÃO declarar `isMonitoringTool`.**
...
**Isto é recomendação de agente, não decisão.** Módulo 50, §5: políticas de loja só existem por
**ADR completo**, e só o fundador aceita. A decisão sobre a flag e sobre a divulgação na descrição
é entregável nominal da Fase 0 (Documento 2, §34.6) e precisa de ADR próprio.
...
| 3 | A **política de Spyware** (answer/14745000), que não examinei, acrescenta requisito ou definição que alcance compartilhamento de localização consentido entre adultos? | Play Console Help, answer/14745000 | idem |
