...
- **Testes de reconciliação de `policy_version`, nos dois regimes do Documento 2, §18.7.2** — eram duas asserções que se excluíam no mesmo cenário, e agora nomeiam casos distintos: **sessão armada offline**, valor local acima do limite vigente é reduzido ao limite na primeira comunicação, com evento e aviso, e nunca abaixo de prazo já vencido; **sessão registrada no servidor**, alteração de limite não encurta o prazo armado, e parâmetro submetido além do máximo é rejeitado com `TIMING_PARAM_OUT_OF_POLICY` preservando o prazo anterior.
...
- Gates backend: lint, typecheck, unit, integração com containers efêmeros, migrations em banco efêmero, verificação de teste negativo de autorização por endpoint, **verificação de linha na tabela de idempotência por endpoint mutante**, scan de imagem e dependências, smoke em staging.
- Secrets scanning e SAST em ambos; achado crítico bloqueia.
- **Gates de marcadores, de listas canônicas e de `modo_teste`: a especificação é a do núcleo §11**, e este módulo não mantém cópia. *(Corrigido na 4ª rodada — ARB4-005: este parágrafo carregava os três na forma anterior à terceira rodada — lista fixa de origens com o §51 dentro, marcadores sem as quatro exigências do gate reescrito, e `modo_teste` como análise de fluxo em vez de invariante de ponto único de despacho. Os três foram reescritos no núcleo justamente por não poderem entrar em operação como estavam, e um agente que os implementasse a partir daqui reconstruiria as versões descartadas. Adota-se o padrão do módulo 50, §11: remissão, não cópia.)*
